...
aws s3 rm s3://tf-state-landing-zone-champ-001/aws-lza/dev/eks-cluster/terraform.tfstate 2>/dev/null || true
aws s3 rm s3://tf-state-landing-zone-champ-001/aws-lza/dev/eks-cluster/terraform.tfstate.backup 2>/dev/null || true
...
echo "State file: $(aws s3 ls s3://tf-state-landing-zone-champ-001/aws-lza/dev/eks-cluster/ 2>&1 | grep -q 'terraform.tfstate' && echo '❌ Still exists' || echo '✅ Gone')"
