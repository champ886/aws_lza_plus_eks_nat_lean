...
See <a href="RUNBOOK.md">RUNBOOK.md</a> for deployment order and gotchas.
...
│   ├── Dev account (435321828725)        → Dev VPC + EKS platform
│   └── Prod account (774386608951)       → Prod VPC (networking only)
│
└── Security OU (ou-77xi-rozu2coh)
    └── Security account (926634327336)   → Shared VPC, NAT, TGW hub
```
...
<tr><td>Management</td><td>501562869247</td><td>root</td><td>Org foundation, Config, logging, IAM Analyzer, GitHub Actions OIDC role, Terraform execution</td></tr>
<tr><td>Security</td><td>926634327336</td><td>Security (ou-77xi-rozu2coh)</td><td>Shared VPC, NAT Gateway, Transit Gateway hub</td></tr>
<tr><td>Dev</td><td>435321828725</td><td>Workload (ou-77xi-o55qljv4)</td><td>Dev VPC, EKS cluster, all workloads</td></tr>
<tr><td>Prod</td><td>774386608951</td><td>Workload (ou-77xi-o55qljv4)</td><td>Prod VPC (networking only, no workloads yet)</td></tr>
</table>

Region: <code>ap-southeast-2</code> everywhere — enforced by SCP. Terraform state: S3 bucket <code>tf-state-landing-zone-champ-001</code>, DynamoDB lock table <code>tf-locks</code>.
...
Why this design over the earlier <code>workflow_run</code> chaining approach: <code>workflow_run</code> fires on *any* completion (including destroy), giving no clean way to prevent a destroy from cascading into the next workflow's apply. Separate forward/reverse orchestrators make that structurally impossible — a destroy can only ever run via the destroy pipeline, in the correct reverse order.
